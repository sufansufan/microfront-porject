      ...
      <div class="btn">
        <div>
          <el-button-group>
            ...
            <el-button type="primary">延期转支出</el-button>
          </el-button-group>
        </div>
      </div>
    </div>
...
import AddBankDetailed from './addBankDetailed'
export default {
  components: { DragDialog, AddBankDetailed },
  data() {
    return {
      count: 30,
      ruleform: {
  ...
  methods: {
    changeTab(status) {
      this.$emit('changeTabBank', 'bankActive', status)
    },
    changeShow(status) {
