  ...
  <div class="import-material">
    ...
    <div>
      <el-table v-loading="loading" :data="tableList" row-key="id" border>
        ...
        <el-table-column prop="name" label="产品名称" align="left" header-align="center">
        ...
        <el-table-column prop="brandName" label="品牌" align="center" width="180">
          <template slot-scope="scope">
            <el-tooltip v-if="scope.row.warning.brandName" class="item" effect="dark" placement="top-start">
              <span slot="content">异常原因<br>{{ scope.row.warning.brandName }}</span>
              <span style="color:red">{{ scope.row.brandName }}</span>
            </el-tooltip>
            <span v-else>{{ scope.row.brandName }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="unit" label="单位" align="center" width="140">
          <template slot-scope="scope">
            <el-tooltip v-if="scope.row.warning.unit" class="item" effect="dark" placement="top-start">
              <span slot="content">异常原因<br>{{ scope.row.warning.unit }}</span>
              <span style="color:red">{{ scope.row.unit }}</span>
            </el-tooltip>
        ...
        <el-table-column prop="model" label="规格型号" align="center">
          <template slot-scope="scope">
            <el-tooltip v-if="scope.row.warning.model" class="item" effect="dark" placement="top-start">
              <span slot="content">异常原因<br>{{ scope.row.warning.model }}</span>
...
<script>
import { importProduct, checkExcel } from '@core/api/materialHouse/product'
  ...
  methods: {
    ...
    handleSuccess(res, file) {
      ...
      this.tableList.forEach(e => {
        e.warning = { name: '', brandName: '', unit: '', model: '' }
      })
      if (errorMap) {
    ...
    submit() {
      ...
      importProduct(this.tableList).then(({ data }) => {
        this.tableList = []
    ...
    errorInfoShow(errorMap, wranList) {
      ...
      errorList.forEach((itemE, index) => {
        tableData.forEach((itemT, indexs) => {
          if (itemE[0].name === itemT.name && itemE[0].brandName === itemT.brandName && itemE[0].unit === itemT.unit && itemE[0].model === itemT.model) {
            itemE.forEach((e, i) => {
              if (i !== 0) {
