...
<template>
  <div class="enginee-insure-add">
    ...
    <el-form ref="ruleForm" :key="key" v-loading="loading" :rules="rules" :model="ruleForm" label-width="135px" class="enginee-form">
      ...
      <el-form-item label="合同金额(保费)" prop="amount">
        <amount-input
          v-model.trim="ruleForm.amount"
          :disabled="disabled"
          placeholder="请输入合同金额(保费)"
          @blur="amountInputBlur('amount')"
          @change="(val) => {handlerChange({type: 'input',selectOptions: {suf:'元'}, formName: 'amount', value: val, name: '合同金额(保费)'})}"
        />
      </el-form-item>
      ...
      <el-form-item label="合同类型" prop="subSubType">
        <el-select
          ...
          placeholder="请选择合同类型"
          :disabled="disabled"
          @change="(val) => {handlerChange({type: 'select', formName: 'subSubType', selectData: 'constant.subSubTypeThree',selectOptions: {code: 'code', name: 'name'}, value: val, name: '合同类型'})}"
        >
          <el-option v-for="item in constant.subSubTypeThree" :key="item.code" :label="item.name" :value="item.code" />
        </el-select>
      </el-form-item>
...
import { insuranceContractSave, insuranceContractEdit, insuranceContractDetails, sonContractVersionDetails, validateUnique } from '@core/api/contractManage/contractmanager'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        amount: [{ required: true, message: '请输入合同金额(保费)', trigger: 'blur' }, { required: true, validator: this.validtorZeroMon, trigger: 'blur' }],
        ...
        subType: [{ required: true, message: '请选择合同类型', trigger: 'change' }],
  ...
  created() {
    ...
    this.$store.dispatch('getConstant', ['EffectiveState', 'ContractCategory', 'BalanceType', 'contractState', 'subSubTypeThree', 'userOrganization', 'generateNo', { type: 'orgList', params: orgListParams }, { type: 'orgPersonList', params: getSessionStorage('orgId') }]).then(() => {
  ...
  methods: {
    ...
    getVersionDetails(id, revisionNumber) {
      ...
      sonContractVersionDetails(id, revisionNumber).then(async({ data }) => {
        ...
        const { approvalNo } = data.subContractDTO
    ...
    getContractDetails(id) {
      this.loading = true
      insuranceContractDetails(id).then(async({ data }) => {
        this.loading = false
        await this.formatterResponseConent(data, 'contract')
    ...
    submit(type) {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          if (this.isAdd) {
            insuranceContractSave(params, type).then(res => {
            ...
            insuranceContractEdit(params, type).then(res => {
