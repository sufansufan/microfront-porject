...
<template>
  <div class="subpackage-add">
    ...
    <el-form ref="ruleForm" :key="key" v-loading="loading" :rules="rules" :model="ruleForm" label-width="100px" class="subpackage-form">
...
import { subContractSave, subContractEdit, subContractDetails, sonContractVersionDetails, validateUnique } from '@core/api/contractManage/contractmanager'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        sendPersonTel: [{ validator: this.validPhoneorMob(), trigger: 'blur' }],
        absolutePeriod: [{ required: true, message: '请输入绝对工期', trigger: 'blur' }, { required: true, validator: this.validateAbsolutePeriodFunc(), trigger: 'blur' }],
        ...
        // cooperateUnitId: [{ required: true, message: '请选择分包公司', trigger: 'change' }],
  ...
  created() {
    ...
    this.$store.dispatch('getConstant', ['EffectiveState', 'ContractCategory', 'BalanceType', 'contractState', 'subSubTypeOne', 'userOrganization', 'generateNo', { type: 'orgList', params: orgListParams }, { type: 'orgPersonList', params: getSessionStorage('orgId') }]).then(() => {
  ...
  methods: {
    ...
    getContractDetails(id) {
      this.loading = true
      subContractDetails(id).then(async({ data }) => {
        this.loading = false
        await this.formatterResponseConent(data, 'contract')
    ...
    submit(type) {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          if (this.isAdd) {
            subContractSave(params, type).then(res => {
            ...
            subContractEdit(params, type).then(res => {
