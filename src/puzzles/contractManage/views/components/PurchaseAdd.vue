...
<template>
  <div class="purchase-add">
    ...
    <el-form ref="ruleForm" :key="key" v-loading="loading" :rules="rules" :model="ruleForm" label-width="135px" class="purchase-form">
      ...
      <el-form-item label="供应商" prop="cooperateUnitId">
        <el-cascader
          ...
          @change="(val) => {handlerChange({type: 'cascader', formName: 'cooperateUnitId', selectData: 'constant.cooperatunitsList',selectOptions: {code: 'id', name: 'name'}, value: val, name: '供应商'})}"
        />
      </el-form-item>
      ...
      <el-form-item label="合同类型" prop="subSubType">
        <el-select
          ...
          placeholder="请选择合同类型"
          :disabled="disabled"
          @change="(val) => {handlerChange({type: 'select', formName: 'subSubType', selectData: 'constant.subSubTypeTwo',selectOptions: {code: 'code', name: 'name'}, value: val, name: '合同类型'})}"
        >
...
import { purchasingContractSave, purchasingContractEdit, purchasingContractDetails, sonContractVersionDetails, validateUnique } from '@core/api/contractManage/contractmanager'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        // cooperateUnitId: [{ required: true, message: '请选择供应商', trigger: 'change' }],
  ...
  created() {
    ...
    //  ''  先不请求这个
    // 'cooperatunitsList',
    this.$store.dispatch('getConstant', ['EffectiveState', 'ContractCategory', 'BalanceType', 'contractState', 'subSubTypeTwo', 'userOrganization', 'generateNo', 'ValuationMethod', 'PaymentMode', { type: 'orgList', params: orgListParams }, { type: 'orgPersonList', params: getSessionStorage('orgId') }]).then(() => {
  ...
  methods: {
    ...
    getContractDetails(id) {
      this.loading = true
      purchasingContractDetails(id).then(async({ data }) => {
        this.loading = false
        await this.formatterResponseConent(data, 'contract')
    ...
    submit(type) {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          if (this.isAdd) {
            purchasingContractSave(params, type).then(res => {
            ...
            purchasingContractEdit(params, type).then(res => {
